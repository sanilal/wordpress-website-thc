...
if ( ! class_exists( 'ReduxFramework_extension_metaboxes' ) ) {
	...
	class ReduxFramework_extension_metaboxes {
		...
		public function init() {
								...
								foreach ( $section['fields'] as $fk => $field ) {
									...
									if ( $addField || ( ( is_admin() && ( $pagenow == "post-new.php" || $pagenow == "post.php" ) ) || ( ! is_admin() ) ) ) {
										...
										if ( isset( $field['output'] ) && ! empty( $field['output'] ) ) {
											...
											// remove duplicate items in metabox, this is a hot fix
											$this->output[ $field['id'] ] = array_unique( $this->output[ $field['id'] ] );
										}
			...
			// remove duplicate items in metabox, this is a hot fix
			$this->parent->sections[ ( count( $this->parent->sections ) - 1 ) ]['fields'] = array_unique( $this->parent->sections[ ( count( $this->parent->sections ) - 1 ) ]['fields'], SORT_REGULAR );
		}
		...
		static function get_current_post_id() {
			return self::static_url_to_postid( ( is_ssl() ? 'https://' : 'http://' ) . $_SERVER["HTTP_HOST"] . $_SERVER["REQUEST_URI"] );
		}
		
		static function static_url_to_postid( $url ) {
					...
					foreach ( $box['post_types'] as $posttype ) {
						if ( isset( $box['title'] ) ) {
							...
							if ( isset( $box['sections'] ) && count( $box['sections'] ) == 1 && isset( $box['sections'][0]['fields'] ) && count( $box['sections'][0]['fields'] ) == 1 && isset( $box['sections'][0]['fields'][0]['title'] ) ) {
								...
								$title = ucfirst( $posttype ) . " " . esc_html__( 'Options', 'floral' );
							}
				...
				<div class="redux-notices">
					<?php if ( $sidebar ) { ?>
						<div class="saved_notice admin-notice notice-blue" style="display:none;">
							<strong><?php echo apply_filters( "redux-imported-text-{$this->parent->args['opt_name']}", esc_html__( 'Settings Imported!', 'floral' ) ) ?></strong>
						</div>
						<div class="redux-save-warn notice-yellow">
							<strong><?php echo apply_filters( "redux-changed-text-{$this->parent->args['opt_name']}", esc_html__( 'Settings have changed, you should save them!', 'floral' ) ) ?></strong>
						</div>
					<?php } ?>
					<div class="redux-field-errors notice-red">
						<strong>
							<span></span> <?php echo esc_html__( 'error(s) were found!', 'floral' ) ?>
						</strong>
					</div>
					<div class="redux-field-warnings notice-yellow">
						<strong>
							<span></span> <?php echo esc_html__( 'warning(s) were found!', 'floral' ) ?>
						</strong>
					</div>
				</div>
				<?php
				echo '<a href="javascript:void(0);" class="expand_options hide" style="display:none;">' . esc_html__( 'Expand', 'floral' ) . '</a>';
				if ( $sidebar ) {
					?>
...
//				return $post_id;
		...
		function meta_boxes_show_errors() {
			if ( isset( $this->notices['errors'] ) && ! empty( $this->notices['errors'] ) ) {
				...
				echo '<p><strong><span></span> ' . count( $this->notices['errors'] ) . ' ' . esc_html__( 'error(s) were found!', 'floral' ) . '</strong></p>';
				echo '</div>';
			}
			...
			if ( isset( $this->notices['warnings'] ) && ! empty( $this->notices['warnings'] ) ) {
				...
				echo '<p><strong><span></span> ' . count( $this->notices['warnings'] ) . ' ' . esc_html__( 'warnings(s) were found!', 'floral' ) . '</strong></p>';
				echo '</div>';
			}
