...
function matthewruddy_image_resize( $url, $width = NULL, $height = NULL, $crop = true, $retina = false ) {
    ...
    if ( empty( $url ) ) {
        return new WP_Error( 'no_image_url', esc_html__( 'No image URL has been entered.', 'floral' ), $url );
    }
    ...
    if ( preg_match( '/\/[0-9]{4}\/[0-9]{2}\/.+$/', $urlinfo['path'], $matches ) ) {
        ...
        $_expo       = explode( '/', WP_CONTENT_DIR );
        $uploads_dir = end( $_expo ) . '/';

        $file_path = ABSPATH . strstr( $urlinfo['path'], $uploads_dir );
    ...
    if ( isset( $info['extension'] ) && isset( $info['dirname'] ) ) {
        $dir = $info['dirname'];
        $ext = $info['extension'];
    } else {
...
//    var_dump( $url );
    ...
    if ( 'bmp' == $ext ) {
        return new WP_Error( 'bmp_mime_type', esc_html__( 'Image is BMP. Please use either JPG or PNG.', 'floral' ), $url );
    }
    ...
    if ( !file_exists( $dest_file_name ) ) {
         ...
         */
        $attached_      = str_replace( trailingslashit( $wp_upload_dir['baseurl'] ), '', $url );
        $query          = $wpdb->prepare( "SELECT * FROM $wpdb->postmeta WHERE meta_key='%s' AND meta_value='%s'", '_wp_attached_file', $attached_ );
        ...
        if ( $meta ) {
            $attached_id = $get_attachment[0]->post_id;
        } else {
            $attached_id = $get_attachment[0]->ID;
        }
        ...
        $metadata = wp_get_attachment_metadata( $attached_id );
        if ( isset( $metadata['image_meta'] ) ) {
            $metadata['image_meta']['resized_images'][] = $resized_width . 'x' . $resized_height;
            wp_update_attachment_metadata( $attached_id, $metadata );
        }
