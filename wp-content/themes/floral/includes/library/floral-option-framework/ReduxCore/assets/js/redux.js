...
(function( $ ) {
    ...
    $.redux.stickyInfo = function() {
		...
		//restyle Redux sticky & Redux footer sticky in two cases normal or RTL
		var isRtl = $("body").hasClass("rtl");
		var stickyFooterRight = 21;
		var reduxStickyLeftOrRight;
		var position;

		if(isRtl) {
			stickyFooterRight = 'inherit';
			reduxStickyLeftOrRight = ($(window).width() - ($('#redux-sticky').offset().left + $('#redux-sticky').outerWidth()));
			position = 'right'
		} else {
			reduxStickyLeftOrRight = $('#redux-sticky').offset().left;
			position = 'left'
		}
            ...
            $( '#redux-footer' ).css(
                {
					...
					bottom  : '0',
					width   : stickyWidth,
					right   : stickyFooterRight
                }
            );
            $( '#redux-footer' ).addClass( 'sticky-footer-fixed' );
			$('.redux-save-warn').css(position, reduxStickyLeftOrRight);
