...
(function ($) {
	...
	redux.field_objects.multi_color = redux.field_objects.multi_color || {};

	$(document).ready(
		function () {
			//redux.field_objects.multi_color.init();
		}
	);

	redux.field_objects.multi_color.init = function (selector) {

		if (!selector) {
			selector = $(document).find('.redux-container-multi_color:visible');
		}
		...
		$(selector).each(
			function () {
				...
				el.find('.redux-multi-color-remove').live(
					'click', function () {
						redux_change($(this));
				...
				el.find('.redux-multi-color-add').click(
					function () {
						...
						for (var i = 0; i < number; i++) {
							...
							el.find('#' + id + ' li:last-child input.regular-text').attr('name', name + '[' + count + '][name]');
							el.find('#' + id + ' li:last-child input.redux-color').val('');
							el.find('#' + id + ' li:last-child input.redux-color').attr('name', name + '[' + count + '][color]');
