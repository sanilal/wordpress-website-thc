...
class Redux_Welcome {
	...
	public function check_version() {
		...
		if ( Redux_Helpers::isLocalHost() ) {
			$compare = true;
		} else if ( class_exists( 'FloralOptionFramework' ) ) {
			$compare = true;
		} else {
...
//            }
	...
	public function change_wp_footer() {
		echo esc_html__( 'If you like <strong>Redux</strong> please leave us a <a href="https://wordpress.org/support/view/plugin-reviews/floral-option-framework?filter=5#postform" target="_blank" class="redux-rating-link" data-rated="Thanks :)">&#9733;&#9733;&#9733;&#9733;&#9733;</a> rating. A huge thank you from Redux in advance!', 'floral' );
	}
	...
	public function support_hash() {
		...
		if ( ! empty( $data['identifier'] ) ) {
			...
			$return['status']  = "error";
			$return['message'] = esc_html__( "Support hash could not be generated. Please try again later.", 'floral' );
		}
	...
	public function admin_menus() {
		...
		$page = 'add_management_' . 'page';
		
		// About Page
		$page(
			esc_html__( 'Welcome to Redux Framework', 'floral' ), esc_html__( 'Redux Framework', 'floral' ), $this->minimum_capability, 'redux-about', array(
				$this,
				'about_screen'
			)
		);
		...
		// Changelog Page
		$page(
			esc_html__( 'Redux Framework Changelog', 'floral' ), esc_html__( 'Redux Framework Changelog', 'floral' ), $this->minimum_capability, 'redux-changelog', array(
				$this,
				'changelog_screen'
			)
		);
		...
		// Support Page
		$page(
			esc_html__( 'Get Support', 'floral' ), esc_html__( 'Get Support', 'floral' ), $this->minimum_capability, 'redux-support', array(
				$this,
				'get_support'
			)
		);
		...
		// Support Page
		$page(
			esc_html__( 'Redux Extensions', 'floral' ), esc_html__( 'Redux Extensions', 'floral' ), $this->minimum_capability, 'floral-option-extensions', array(
				$this,
				'redux_extensions'
			)
		);
		...
		// Credits Page
		$page(
			esc_html__( 'The people that develop Redux Framework', 'floral' ), esc_html__( 'The people that develop Redux Framework', 'floral' ), $this->minimum_capability, 'redux-credits', array(
				$this,
				'credits_screen'
			)
		);
		...
		// Status Page
		$page(
			esc_html__( 'Redux Framework Status', 'floral' ), esc_html__( 'Redux Framework Status', 'floral' ), $this->minimum_capability, 'redux-status', array(
				$this,
				'status_screen'
			)
		);
		...
		remove_submenu_page( 'tools.php', 'floral-option-extensions' );
		...
		<h2 class="nav-tab-wrapper">
			<a class="nav-tab <?php echo ( $selected == 'redux-about' ) ? 'nav-tab-active' : ''; ?>"
			   href="<?php echo esc_url( admin_url( add_query_arg( array( 'page' => 'redux-about' ), 'tools.php' ) ) ); ?>">
				<?php esc_attr_e( "What's New", 'floral' ); ?>
			</a> <a class="nav-tab <?php echo ( $selected == 'floral-option-extensions' ) ? 'nav-tab-active' : ''; ?>"
					href="<?php echo esc_url( admin_url( add_query_arg( array( 'page' => 'floral-option-extensions' ), 'tools.php' ) ) ); ?>">
				<?php esc_attr_e( 'Extensions', 'floral' ); ?>
				...
				<?php esc_attr_e( 'Changelog', 'floral' ); ?>
	...
	public function parse_readme() {
		...
		return '<script src="' . 'http://gist-it.appspot.com/https://github.com/reduxframework/floral-option-framework/blob/master/CHANGELOG.md?slice=2:0&footer=0">// <![CDATA[// ]]></script>';
	...
	public function contributors() {
		...
		foreach ( $contributors as $contributor ) {
			...
			$contributor_list .= sprintf( '<a href="%s" title="%s" target="_blank">', esc_url( 'https://github.com/' . $contributor->login ), esc_html( sprintf( esc_html__( 'View %s', 'floral' ), esc_html( $contributor->login ) ) )
			);
	...
	public function get_contributors() {
		...
		$response = wp_remote_get( 'https://api.github.com/repos/ReduxFramework/floral-option-framework/contributors', array( 'sslverify' => false ) );
