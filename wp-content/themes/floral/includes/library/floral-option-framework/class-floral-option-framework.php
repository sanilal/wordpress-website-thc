<?php
    ...
    if ( ! class_exists( 'FloralOptionFramework' ) ) {
        ...
        class FloralOptionFramework {
             ...
             * @since       3.0.0
             */
            protected $plugin_slug = 'floral-option-framework';
            ...
            /**
             * @access      private
             * @var         \FloralOptionFramework $instance The one true ReduxFrameworkPlugin
             * @since       3.0.0
             */
             ...
             * @access      public
             * @since       3.1.3
             * @return      FloralOptionFramework::$instance The one true ReduxFrameworkPlugin
             */
            ...
            public function get_redux_options() {
                ...
                if ( is_multisite() ) {
                    ...
                    foreach ( $plugins as $file => $plugin ) {
                        if ( strpos( $file, 'floral-option-framework.php' ) !== false ) {
            ...
            public function includes() {
                ...
                if ( file_exists( dirname( __FILE__ ) . '/ReduxCore/floral-option-extensions/config.php' ) ) {
                    require_once dirname( __FILE__ ) . '/ReduxCore/floral-option-extensions/config.php';
                }
            ...
            public function load_first() {
                ...
                $path = str_replace( 'class-floral-option-framework.php', 'floral-option-framework.php', $path );
            ...
            private static function single_activate() {
                ...
                $notices[] = esc_html__( 'Redux Framework has an embedded demo.', 'floral' ) . ' <a href="./plugins.php?ReduxFrameworkPlugin=demo">' . esc_html__( 'Click here to activate the sample config file.', 'floral' ) . '</a>';
            ...
            public function add_action_links( $links ) {
                ...
                /**
                 * return array_merge(
                 *      array( 'redux_plugin_settings' => '<a href="' . admin_url( 'plugins.php?page=' . 'redux_plugin_settings' ) . '">' . esc_html__( 'Settings', 'floral' ) . '</a>' ),
                 *      $links
                 * );
                 */
            }
            ...
            public function plugin_metalinks( $links, $file ) {
                if ( strpos( $file, 'floral-option-framework.php' ) !== false && is_plugin_active( $file ) ) {

                    $new_links = array(
                        '<a href="' . 'http://' . 'docs.reduxframework.com/" target="_blank">' . esc_html__( 'Docs', 'floral' ) . '</a>',
                        '<a href="' . 'https://' . 'github.com/ReduxFramework/floral-option-framework" target="_blank">' . esc_html__( 'Repo', 'floral' ) . '</a>',
                        '<a href="' . 'http://' . 'build.reduxframework.com/" target="_blank">' . esc_html__( 'Builder', 'floral' ) . '</a>',
                        '<a href="' . admin_url( 'tools.php?page=redux-support' ) . '">' . esc_html__( 'Get Support', 'floral' ) . '</a>',
                    );

                    if ( ( is_multisite() && $this->plugin_network_activated ) || ! is_network_admin() || ! is_multisite() ) {
                        if ( $this->options['demo'] ) {
                            $new_links[3] .= '<br /><span style="display: block; padding-top: 6px;"><a href="./plugins.php?ReduxFrameworkPlugin=demo" style="color: #bc0b0b;">' . esc_html__( 'Deactivate Demo Mode', 'floral' ) . '</a></span>';
                        } else {
                            $new_links[3] .= '<br /><span style="display: block; padding-top: 6px;"><a href="./plugins.php?ReduxFrameworkPlugin=demo" style="color: #bc0b0b;">' . esc_html__( 'Activate Demo Mode', 'floral' ) . '</a></span>';
                        }
                    }
