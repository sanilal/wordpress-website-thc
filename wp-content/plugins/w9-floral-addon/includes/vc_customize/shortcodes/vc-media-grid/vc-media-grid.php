...
class Floral_SC_Media_Grid extends Floral_Basic_Grid {
    const SC_BASE = 'vc_media_grid';
    ...
    public function setItemsIfEmpty( $items ) {
        ...
        if ( empty( $items ) ) {
            ...
            $item = '[vc_gitem]' . Floral_Image::get_placeholder_image() . '[/vc_gitem]';
    ...
    public function singleParamHtmlHolder( $param, $value ) {
        ...
        if ( 'include' === $param_name ) {
            ...
            foreach ( $images_ids as $image ) {
                ...
                $output .= ( $img ? '<li>' . $img['thumbnail'] . '</li>' : '<li><img width="150" height="150" test="' . $image . '" src="' . vc_asset_url( 'vc/blank.gif' ) . '" class="attachment-thumbnail" alt="' . esc_attr( 'Attachment thumbnail', 'w9-floral-addon' ) . '" title="" /></li>' );
            }
            $output .= '</ul>';
            $output .= '<a href="#" class="column_edit_trigger' . ( !empty( $images_ids ) ? ' image-exists' : '' ) . '">' . __( 'Add images', 'w9-floral-addon' ) . '</a>';
