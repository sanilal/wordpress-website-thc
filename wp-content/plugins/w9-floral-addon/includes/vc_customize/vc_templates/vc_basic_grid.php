 ...
 * @filename: vc_basic_grid.php
 * @time    : 9/19/16 4:49 PM
 ...
 */
$this->post_id = false;
$css           = $el_class = $element_auto_responsive = '';
$posts         = $filter_terms = array();
$this->buildAtts( $atts, $content );

$css                     = isset( $atts['css'] ) ? $atts['css'] : '';
$el_class                = isset( $atts['el_class'] ) ? $atts['el_class'] : '';
$element_auto_responsive = isset( $atts['element_auto_responsive'] ) ? $atts['element_auto_responsive'] : '';

$class_to_filter = 'vc_grid-container vc_clearfix wpb_content_element ' . $this->shortcode;
if ( isset( $atts['pagination_position'] ) ) {
    if ( empty( $atts['pagination_position'] ) ) {
        $atts['pagination_position'] = 'pagination-position-30px';
    }
    $class_to_filter .= ' ' . $atts['pagination_position'];
}
$class_to_filter .= vc_shortcode_custom_css_class( $css, ' ' ) . $this->getExtraClass( $el_class );
$css_class = apply_filters( VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, $class_to_filter, $this->settings['base'], $atts );
if($element_auto_responsive === 'yes'){
    $css_class .= ' floral-auto-responsive-grid-item';
}
