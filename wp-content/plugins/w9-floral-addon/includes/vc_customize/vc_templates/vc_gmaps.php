 ...
 */
$title  = $link = $size = $filter = $filter_value = $el_class = $css = '';
$output = '';
...
$inline_css = array();
if ( !empty( $filter ) ) {
    $inline_css[] = sprintf( '-webkit-filter: %1$s(%2$s)', $filter, $filter_value );
    $inline_css[] = sprintf( 'filter: %1$s(%2$s)', $filter, $filter_value );
}
?>
<div class="<?php echo esc_attr( $css_class ); ?>">
    <?php echo wpb_widget_title( array( 'title' => $title, 'extraclass' => 'wpb_map_heading' ) ); ?>
    <div class="wpb_wrapper">
        <div class="wpb_map_wraper" <?php echo Floral_Map_Helpers::get_inline_style( $inline_css ); ?>>
            <?php
