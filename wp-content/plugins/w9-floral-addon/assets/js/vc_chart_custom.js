!function ($) {
	function VcChart(element, options) {
		this.el = element, this.$el = $(this.el), this.options = $.extend({
			color         : "#f7f7f7",
			units         : "",
			label_selector: ".vc_pie_chart_value .__inner .__value",
	...
	VcChart.prototype = {
		constructor     : VcChart, _progress_v: 0, animated: !1, init: function () {
			...
			var radius, w = this.$el.addClass("vc_ready").width(), border_w = Math.abs(Number(this.$el.data("pie-border-width") ));
			console.log(border_w);
			w || (w = this.$el.parents(":visible").first().width() - 2), w_dont_need_this_var = w / 100 * 80, radius = w / 2 - border_w - 1, this.$wrapper.css({width: w + "px"}), this.$label.css({
				// width        : w,
				// height       : w,
				// "line-height": w + "px"
